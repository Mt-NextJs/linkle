...
export default function StylePreview({
  ...
  const [displayImgUrl, setDisplayImgUrl] = useState<string>(linkImg);
  const [hasImgError, setHasImgError] = useState(false);

  // 이미지 URL 유효성을 확인하는 함수
  const checkImageExists = async (url: string) => {
    try {
      ...
      return response.ok; // 이 경우, 항상 true일 가능성이 높습니다.
    } catch {
      return false;
    }
  };

  // 이미지 URL이 변경될 때마다 이미지 유효성을 검사
  useEffect(() => {
    if (linkImg && isValidUrl(linkImg)) {
      checkImageExists(linkImg).then((exists) => {
        if (exists) {
          setDisplayImgUrl(linkImg);
          setHasImgError(false);
          setIsImgUrlConnectionError(false);
        } else {
          setDisplayImgUrl(placeholderImage);
          setHasImgError(true);
          setIsImgUrlConnectionError(true);
        }
      });
  ...
  }, [linkImg, setIsImgUrlConnectionError, isValidUrl]);
